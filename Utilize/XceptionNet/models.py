...
def return_pytorch04_xception(pretrained=True):
    ...
    if pretrained:
        ...
        state_dict = torch.load(
            'Utilize/XceptionNet/xception-b5690688.pth')
...
def load_pretrain_xception(model):
    # Raises warning "src not broadcastable to dst" but thats fine
...
class XceptionTransferModel(Xception):

    def __init__(self, num_out_classes = 2, dropout = 0.0):
        super().__init__()

        self = load_pretrain_xception(self)

        num_ftrs = self.fc.in_features

        self.fc = nn.Linear(num_ftrs, num_out_classes)
        #
        for i, param in self.named_parameters():
            param.requires_grad = False

    def set_layer_trainable(self, layer_name):
...
class TransferModel(nn.Module):
    ...
    def __init__(self, modelchoice, num_out_classes=2, dropout=0.0):
        ...
        self.extra_layer = nn.Linear(500, 2)
